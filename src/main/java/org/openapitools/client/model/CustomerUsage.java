/*
 * Lago API documentation
 * Lago API allows your application to push customer information and metrics (events) from your application to the billing application.
 *
 * The version of the OpenAPI document: 1.28.1
 * Contact: tech@getlago.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package org.openapitools.client.model;

import java.util.Objects;
import java.util.Arrays;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonTypeName;
import com.fasterxml.jackson.annotation.JsonValue;
import org.openapitools.client.model.CustomerUsageObject;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;
import com.fasterxml.jackson.annotation.JsonTypeName;

/**
 * CustomerUsage
 */
@JsonPropertyOrder({
  CustomerUsage.JSON_PROPERTY_CUSTOMER_USAGE
})
@javax.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", date = "2025-05-23T15:05:45.180675+03:00[Europe/Kiev]", comments = "Generator version: 7.13.0")
public class CustomerUsage {
  public static final String JSON_PROPERTY_CUSTOMER_USAGE = "customer_usage";
  @javax.annotation.Nonnull
  private CustomerUsageObject customerUsage;

  public CustomerUsage() {
  }

  public CustomerUsage customerUsage(@javax.annotation.Nonnull CustomerUsageObject customerUsage) {
    
    this.customerUsage = customerUsage;
    return this;
  }

  /**
   * Get customerUsage
   * @return customerUsage
   */
  @javax.annotation.Nonnull
  @JsonProperty(JSON_PROPERTY_CUSTOMER_USAGE)
  @JsonInclude(value = JsonInclude.Include.ALWAYS)

  public CustomerUsageObject getCustomerUsage() {
    return customerUsage;
  }


  @JsonProperty(JSON_PROPERTY_CUSTOMER_USAGE)
  @JsonInclude(value = JsonInclude.Include.ALWAYS)
  public void setCustomerUsage(@javax.annotation.Nonnull CustomerUsageObject customerUsage) {
    this.customerUsage = customerUsage;
  }


  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    CustomerUsage customerUsage = (CustomerUsage) o;
    return Objects.equals(this.customerUsage, customerUsage.customerUsage);
  }

  @Override
  public int hashCode() {
    return Objects.hash(customerUsage);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class CustomerUsage {\n");
    sb.append("    customerUsage: ").append(toIndentedString(customerUsage)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

